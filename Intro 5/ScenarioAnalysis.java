...
public class ScenarioAnalysis {
    ...
    public void computeCO2EmissionsAndCost () {
        ...
        double co2Gas = Fuel.CO2EMITTED_GASCOMBUSTION;             // 8.887 kg of CO2 released with the combustion of each gallon of gasoline
        double co2Electric = Fuel.CO2EMITTED_GENERATEmWh;          //// 998.4 lbs of CO2 are emitted to generate 1 mWh on average
        ...
        for (int i = 0; i < vehicles.length; i++) {
            ...
            totalEmission = totalMiles / milesPerVehicles * co2Gas;
            inputCost = getGasPrice();
            }
            else if (vehicles[i].getFuel().getType() == 2){
                totalEmission = (totalMiles/milesPerVehicles) * (vehicles[i].getFuel().getKWhPerCharge()) * (co2Electric/1000) * (0.45);
                inputCost = getElectricityPrice() * (vehicles[i].getFuel().getKWhPerCharge()) / 100;   
            } 
            vehicles[i].setCO2Emission(totalEmission);
            double totalinputCost = (totalMiles / milesPerVehicles) * inputCost;
            vehicles[i].setFuelCost(totalinputCost);
