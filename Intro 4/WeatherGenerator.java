...
public class WeatherGenerator {
    ...
    int month, double[][] drywet, double[][] wetwet) {
        ...
        int wetrow = 0;
        int dryrow = 0; 
        for (int w = 0; w<wetwet.length; w++){
            if (longitute == wetwet[w][0]){
                if (latitude == wetwet[w][1]){
                    wetrow = w;
                    break;
                }
            }
        }
        for (int w = 0; w<drywet.length; w++){
            if (longitute == drywet[w][0]){
                if (latitude == drywet[w][1]){
                    dryrow = w;
                    break;
                }
            }
        ...
        double drypred = drywet[dryrow][m];
        double wetpred = wetwet[wetrow][m];
        for (int d = 1; d<days; d++){
            if (forecast[d-1] == WET){
                if (StdRandom.uniform() <= wetpred){
                    forecast[d] = WET;
                }
                else{
                    forecast [d] = DRY;
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        int counter = 0;
        int maximum = 0;
        for (int w=0; w<forecast.length; w++){
            if (forecast[w]== mode){
                counter++;
            }
            else {
